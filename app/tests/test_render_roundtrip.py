...
def test_render_news_clean(fake_repo: Path):
    ...
    # using the "In plain language: …" prefix instead of `eli14:`.
...
def test_lint_flags_legacy_corpus():
    """Confirm the lint catches contamination in the LEGACY 2026-05-04 corpus.

    Phase 3 only adds the gate; Phase 4 cleans the corpus. This proves
    the gate fires on a known-bad input, justifying the gate's
    existence.
    """
    legacy_news = (
        REPO_ROOT_REAL / "report" / "en" / "news-20260504.md"
    )
    if not legacy_news.is_file():
        pytest.skip(
            "legacy 2026-05-04 corpus not present; skipping lint regression"
        )
    rc = lint_markdown_clean.lint_paths([legacy_news])
    assert rc == 1, (
        "expected lint to FAIL on legacy mixed-format markdown "
        "(eli14, **Summary:**, etc.) — the gate may be too lenient."
    )
