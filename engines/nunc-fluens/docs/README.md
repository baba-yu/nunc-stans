...
renders, gates, and writes a daily news board into a **data
instance** it owns (a plain local directory of markdown + JSON, with a
disposable `store/` for the SQLite working cache and run logs — the
product is fully local and git-less). It is a self-contained product;
the monorepo's Formans world view reads one instance's exports
strictly read-only.
...
- `pipeline/instance-template/` — the instance TEMPLATE: directory
  skeleton (`data/…` incl. the empty citation ledger
  `data/history/reference-history.log`), synthetic editorial seeds
  (`news-topics.md`, `citation-restrictions.md`, `glossary.yml`), and
  the README seed. `nunc-fluens init` stamps data instances from it.
...
- `dashboard/` — the reader-facing dashboard (static `index.html` +
  ...
  deploy/stage time.
- `instances/` — the default (monorepo-gitignored) home for data
  instances: one plain data directory per profile, stamped by
  `nunc-fluens init`.
...
copies their data into an instance (source read-only; the import is
recorded in `instance.json`) — that command is the only place
old-shape knowledge survives.

## The instance layout (v2)

An instance is a plain local data directory carrying an
`instance.json` birth stamp, `data/sourcedata/` (per-day research
inputs, incl. locale fan-outs and each day's `run.json`), the daily
quartet `data/daily-news/`, `data/future-prediction/`,
`data/history/` (dormant pool, theme reviews, weekly snapshots, and
the citation ledger `reference-history.log`), `data/reference/`, plus
`data/exports/` (graphs + manifest + snapshots + prefix-tokens),
`data/archives/` (aged-out snapshots, disposable), and `README*.md` at
the root. Runtime state lives in a disposable `store/`:
`world/analytics.sqlite`, `runs/ai-runs.jsonl`, and an optional
`news-config.json` override (when present it wins over the main
store's copy that the gate/drawer edit). The view side reads
`data/exports/` only. The pipeline never creates or requires git in an
instance — if you choose to version one yourself, ignore `store/` and
`data/archives/`.
