...
The two fill paths are complementary: upsert overwrites YAML-owned fields (definitions, locale fan-out, canonical_link, status, reviewed_by_human) without touching SQLite-owned counters/dates. See `design/skills/extract-glossary-candidates.md` § "Seed modes" for the field split.
