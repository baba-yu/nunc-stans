...
Daily flow that updates every README in the run's render set — EN plus the news-config `locales` subset, default `{ja, es, fil}` — with the 3-day window and rebuilds the dashboard data. Phase 3 rewrite (≤ 80 lines): the inline checks live in skill READMEs.
...
2. **update-readme-3day-window** *(LLM)* — for each locale `<L>` in this run's render set (EN + news-config `locales`, default `{ja, es, fil}`), update `README<.L>.md` with the last 3 days (today + 2 prior). Yesterday's section can be copy-pasted from previous run; the day before that gets pushed out. **Locale-link routing rule**: link to `data/daily-news/<L>/news-…md` and `data/future-prediction/<L>/future-prediction-…md` when those files exist on disk; fall back to `…/en/…` only when the locale file is genuinely missing. Section H2 / H3 headers stay English in every README; only bullet text + link labels translate. **Dispatch shape: one sub-agent per locale** (in parallel). The parent only holds the per-locale returned status lines — never the full READMEs in working memory.

3. **link-routing check** *(deterministic)* — implemented in `pipeline/src/orchestrator/readme-checks.ts`: walks every link and flags `/en/` references that should have been the locale's own file.
...
5. **`update-pages`** *(shared)* — score + export. Rebuilds `data/exports/graph-*.json` from `store/world/analytics.sqlite`. On `database disk image is malformed` it auto-deletes + retries. On non-zero exit, abort.
...
1. Every README in the render set ends in `\n---\n` and passes the link-routing + structural completeness checks.
2. `data/exports/manifest.json` lists exactly the run's locales (EN + the configured set) and `default_locale='en'`.
