...
if git grep -nI "nunc-stans-data" -- . ':!tools/check.sh' ':!design/*' ':!**/prd-override.md' >/dev/null 2>&1; then
  echo "NG FD-3.2: 'nunc-stans-data' has leaked into code/config"; fail=1
else
  echo "ok FD-3.2: no vault path leak"
fi

# FD-7.4 (foundation): naive detection of engine-to-engine imports / references
# from frontend into engine internals. Pre-v1 Phase 0 is a minimal
