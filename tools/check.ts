...
// FD-7.4: frontend and agents must not reference engine internals
const imports = gitGrep(['-E', 'engines/(nunc-fluens|nunc-stans|fourfive)', '--', 'frontend', 'agents'])
const bad = imports.split('\n').filter(l => l && !l.includes('contracts/'))
if (bad.length) ng(`import: frontend/agents reference engine internals\n${bad.join('\n')}`)
else ok('import: frontend/agents→contracts only (so far)')
