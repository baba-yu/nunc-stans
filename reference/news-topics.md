...
| Bay Area / SV AI meet-up events | **Any AI industry event** — vendor-hosted events (Summits / DevDays / developer days / launch events / partner conferences), conferences with AI tracks (RSA / DEF CON / Black Hat / etc.), hackathons, Bay Area meet-ups. The 'meet-up' framing is shorthand; treat the topic as 'AI industry event coverage'. **Do NOT restrict to a fixed vendor list** — any vendor (established or newly emerged: Zenity, Lakera, Robust Intelligence, Adversa, or whoever hosts an event next month) is in scope. Self-anchored event brands (AI Engineer Summit, GitHub Universe, re:Invent, Snowflake Summit, Latent Space meet-ups) are examples of recognizable event names, but the category is *event-ness*, not *brand-list*. |
